...
const CollectionsPanel = () => {
            ...
            if (response.success && Array.isArray(response.collections)) {
                ...
                const collectionsWithRequests = await Promise.all(
                    response.collections.map(async (collection) => {
                        try {
                            const requestsResponse = await apiService.getRequests(collection.id);
                            return {
                                ...collection,
                                requests: requestsResponse.success && Array.isArray(requestsResponse.requests)
                                    ? requestsResponse.requests
                                    : [],
                            };
                        } catch (error) {
                            console.error(`Error loading requests for collection ${collection.id}:`, error);
    ...
    const handleDeleteCollection = async (collectionId) => {
        if (window.confirm('Are you sure you want to delete this collection? This will also delete all folders and requests inside it.')) {
            try {
                if (import.meta.env.DEV) {
                ...
                const errorMsg = error.response?.data?.message || error.message || 'Failed to delete collection';
                toast.error(errorMsg);
    ...
    const handleCreateRequest = async () => {
        ...
        try {
            ...
            const response = await apiService.createRequest(selectedCollectionId, requestData);
                                ...
                                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', pr: 1 }}>
                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flex: 1, minWidth: 0 }}>
                                        {expandedCollections.includes(collection.id) ? (
                                            <FolderOpen sx={{ fontSize: 16, color: '#22c55e', flexShrink: 0 }} />
                                        ) : (
                                            <Folder sx={{ fontSize: 16, color: '#22c55e', flexShrink: 0 }} />
                                        )}
                                        <Typography 
                                            variant="body2" 
                                            sx={{ 
            ...
            >
                ...
                <DialogContent>
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
                        ...
                        {(newRequest.method === 'POST' || newRequest.method === 'PUT' || newRequest.method === 'PATCH') && (
                            <>
                                <Alert severity="info" sx={{ fontSize: '0.875rem' }}>
                                    Body must be valid JSON format for {newRequest.method} requests
                                </Alert>
                                <TextField
                                    label="Body (JSON)"
                                    fullWidth
